...
const OrderEdit = ({id, modalIsOpen, closeModal}) => {
    ...
    const getOrderDetail = async () => {
        try {
            ...
            let ids = data.result.orderDetails[0].productSupplyId
            let productSupplyConditionId = data.result.orderDetails[0].productSupplyConditionId
            if (productSupplyConditionId !== null) {
                const {data, status} = await GetProductSupplyConditions(ids)
                // setProductSupplyConditions(data.result.productSupplyConditions.values)
                let condition = data.result.productSupplyConditions.values

                let ff = condition.filter(item => item.id === Order[0].productSupplyConditionId)
                const merged = ff.map(item =>
                    ({
